...
def face_encodings_and_landmarks(face_image, known_face_locations=None, num_jitters=1, model="large"):
    """
    Given an image, return the 128-dimension face encoding and landmarks for each face in the image.
    ...
    landmarks = np.array([[(p.x, p.y) for p in landmark.parts()] for landmark in raw_landmarks])
